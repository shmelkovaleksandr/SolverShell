...
public:
     ...
     * @param shell Объект типа SOSH_Shell.
     */
    SOSH_Parser(SOSH_Lexer lexer, SOSH_Shell shell = SOSH_Shell("")) : lexer(lexer), currentPosition(0), shell(shell) {};
...
private:
    SOSH_Lexer lexer; ///< Лексер для получения токенов.
     ...
     */
    SOSH_ASTNode* parseExpressionOrFunction(const std::vector<SOSH_Token>& tokens);
     ...
     * @return Указатель на узел абстрактного синтаксического дерева (AST).
     */
    SOSH_ASTNode* parseExpression(const std::vector<SOSH_Token>& tokens);
     ...
     * @return Указатель на узел абстрактного синтаксического дерева (AST).
     */
    SOSH_ASTNode* parseTerm(const std::vector<SOSH_Token>& tokens);
     ...
     * @return Указатель на узел абстрактного синтаксического дерева (AST).
     */
    SOSH_ASTNode* parseFactor(const std::vector<SOSH_Token>& tokens);
};
